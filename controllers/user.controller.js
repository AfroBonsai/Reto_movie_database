...
UserController.logIn = (req, res) => {
  ...
  let { email, password } = req.body; // Declare email and password for cross check
  ...
  User.findOne({ email: email }).then(user => { // Search for an existing user
    if (!user) { // Check if the email exists to search for it
      ...
      if (bcrypt.compareSync(password, user.password)) {
        let token = jwt.sign({ user: user }, authConfig.secret, { expiresIn: authConfig.expires }); // Creates the token
        res.json({
          user: user,
          token: token
        })
...
UserController.update = (req, res) => {
  ...
  if (req.user.user.superUser == true) { // Check if the logged in user has admin privileges
    ...
    if (!req.body) { // Verifies if the input parameters match the required ones
      return res.status(400).send({ message: "Data to update can not be empty!" });
    };

    User.findByIdAndUpdate(_id, req.body, { useFindAndModify: false }) // Finds the user by ID then updates
      .then(data => {
        if (!data) {
          res.status(404).send({ message: `Cannot update user with id=${_id}. Maybe user was not found!` });
        } else res.send({ message: "User was updated successfully." });
      })
      .catch(err => {
        res.status(500).send({ message: `Error updating User with id= ${_id}. The email you introduced is already registered` });
      });
  }
  else {
    res.send({ message: "You are not authorized to modify user entries." }); //Error in case token is invalid
  }
};
...
UserController.delete = (req, res) => { // Deletes an entry

  if (req.user.user.superUser == true) { // Check if the logged in user has admin privileges

    const _id = req.params._id; // Declare ID to use as search parameter

    User.findByIdAndRemove(_id, { useFindAndModify: false }) // Finds the user by ID then deletes
      .then(data => {
        if (!data) {
          res.status(404).send({ message: `Cannot delete user with id=${_id}. Maybe user was not found!` });
        } else {
          res.send({ message: "User was deleted successfully!" });
        }
      })
      .catch(err => {
        res.status(500).send({ message: "Could not delete user with id=" + _id });
      });
  }
  else {
    res.send({ message: "You are not authorized to delete user entries." }); //Error in case token is invalid
  }
};
...
UserController.deleteAll = (req, res) => { // Deletes ALL user entries

  if (req.user.user.superUser == true) { // Check if the logged in user has admin privileges

    User.deleteMany({})
      .then(data => {
        res.send({ message: `ALL (${data.deletedCount}) users were deleted successfully!` });
      })
      .catch(err => {
    ...
    res.send({ message: "You are not authorized to delete ALL user entries." });  //Error in case token is invalid
  }
};
